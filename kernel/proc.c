...
found:
  ...
  kpgmap(p->kpagetable, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
  p->kstack = va;
...
int growproc(int n) {
  ...
  if (n > 0) {
    ...
    if (copypage(p->pagetable, p->kpagetable, p->sz, sz) < 0) {
      return -1;
    }
  } else if (n < 0) {
    sz = uvmdealloc(p->pagetable, sz, sz + n);
    ukvmunmap(p->kpagetable, PGROUNDUP(sz),
              (PGROUNDUP(p->sz) - PGROUNDUP(sz)) / PGSIZE);
  }
  p->sz = sz;
...
int fork(void) {
  ...
  if (copypage(np->pagetable, np->kpagetable, 0, p->sz) < 0) {
    freeproc(np);
    release(&np->lock);
    return -1;
  }
